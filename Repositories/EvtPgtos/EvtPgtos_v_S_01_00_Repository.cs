...
{
    public class EvtPgtos_v_S_01_00_Repository
    {
       ...
       @ideEvento_nrRecibo, @ideEvento_indApuracao, @ideEvento_perApur, @ideEvento_tpAmb, @ideEvento_procEmi, 
       ...
       @recibo_nrRecibo, @recepcao_processamento_dhProcessamento, @versao_layout_evtPgtos, @id_cadastro_envios, @nome_arquivo_importado)";
                        ...
                        if (verifica_InfoPgto != null && verifica_InfoPgto.Count > 0)
                        {
                            foreach (var item_infoPgto in verifica_InfoPgto)
                            {
                                ...
                                var infoPgto_indResBr = "";
                                long id_evtpgtos_informacoes = 0;

                                using (MySqlConnection connection = new MySqlConnection(connectionString))
                                {
                                    connection.Open();
                                    var sql = "INSERT INTO s_1210_evtpgtos_informacoes (id_projeto, id_usuario, " +
                                        "id_cad_evtpgtos, infoPgto_dtPgto, infoPgto_tpPgto, infoPgto_indResBr, id_cadastro_envios) VALUES" +
                                        "(@id_projeto, @id_usuario, @id_cad_evtpgtos, @infoPgto_dtPgto, @infoPgto_tpPgto, @infoPgto_indResBr, " +
                                        "@id_cadastro_envios)";
                                ...
                                var detPgtoFl_nrRecArq = "";
                                var detPgtoFl_indPgtoTt = "";

                                using (MySqlConnection connection = new MySqlConnection(connectionString))
                                {
                                    connection.Open();
                                    var sql = "INSERT INTO s_1210_evtpgtos_info_detpgtofl (id_projeto, id_usuario, " +
                                        "id_cad_evtpgtos, id_evtpgtos_informacoes, detPgtoFl_perRef, detPgtoFl_ideDmDev, detPgtoFl_indPgtoTt, " +
                                        "detPgtoFl_vrLiq, detPgtoFl_nrRecArq, id_cadastro_envios)VALUES(@id_projeto, @id_usuario, " +
                                        "@id_cad_evtpgtos, @id_evtpgtos_informacoes, @detPgtoFl_perRef, @detPgtoFl_ideDmDev, " +
                                    ...
                                    using (MySqlCommand cmd = new MySqlCommand(sql, connection))
                                    {
                                        ...
                                        cmd.Parameters.AddWithValue("@detPgtoFl_perRef", infoPgto_perRef ?? "");
                                        cmd.Parameters.AddWithValue("@detPgtoFl_ideDmDev", infoPgto_ideDmDev);
                                        cmd.Parameters.AddWithValue("@detPgtoFl_indPgtoTt", detPgtoFl_indPgtoTt);
                                        cmd.Parameters.AddWithValue("@detPgtoFl_vrLiq", infoPgto_vrLiq ?? "");
